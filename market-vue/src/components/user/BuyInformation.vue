<template>
  <div>
    ...
    <div class="goodContent">
      ...
      <p style="font-weight:bold;font-size: 25px;color: #21675e;
    position: absolute;top:63%;left:55%;margin-left: 6vw">
        预期价格：{{ goodProperty.price }}元
      </p>
...
//获取购物车最大id
    ...
    getMaxLeaId2(){
      ...
      }).then(res=>res.data).then(res=>
      {
        ...
        if(res.resultCode==200)
        {
          console.log("MAXLEAID1")
          console.log(this.MaxLeaId)
          this.MaxleaId = res.data;
          console.log(this.MaxLeaId)
        }
        else{
          console.log("失败")
    ...
    getMaxLeaId3(){
      ...
      console.log("getMaxLeaId")
      this.$axios({
        url:this.$http + '/leave-messages/toGetMaxId',
        method:'get',
        headers:{
          token:localStorage.getItem("token")
        }
      }).then(res=>res.data).then(res=>
      {
        ...
        if(res.resultCode==200)
        {
          ...
          _this.MaxLeaId = Math.max(..._this.allMessages.map(lea => lea.commentId));
          console.log("MAXLEAID1")
