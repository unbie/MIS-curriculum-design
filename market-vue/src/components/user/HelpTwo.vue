  ...
  <div>
    ...
    <div>
      <img height="24"
           ...
           width= "24">
      <a style="color: #21675e;float: left;margin-left:1%;margin-top:1%">当前一共搜索到{{ this.count }}条有关消息</a>
    </div>
    ...
    <div class="pagination" style="position: absolute;top:58%;left:10%">
      <el-button v-for="page in displayedPages" :key="page"
                 @click="changePage(page)" :class="{ active: page === currentPage }">{{ page }}</el-button>
    </div>
...
export default {
  ...
  methods:{
    ...
    search1() {
      ...
      // 假设使用axios发送异步请求
      this.$axios({
        url: this.$http + '/good/search?keyword=' + keyword + '&isSale=0',
        method: 'get',
        headers: {
          token: localStorage.getItem("token")
        }
    ...
    searchByCategory1() {
      const keyword = this.$route.query.keyword;
      this.$axios({
        url: this.$http + '/good/searchByCategory?categoryId=' + keyword + '&isSale=0',
        method: 'get',
        headers: {
          token: localStorage.getItem("token")
        }
